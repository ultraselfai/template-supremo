...
export function Step9RankingTipografias({
  ...
  const orderedReferences = useMemo(() => {
    ...
    return idsToUse
      .map((id) => TYPOGRAPHY_REFERENCES.find((ref) => ref.id === id))
      .filter((ref): ref is TypographyReference => ref !== undefined);
  }, [ranking, likedIds]);

  const handleReorder = (newItems: TypographyReference[]) => {
    ...
    return (
      <div>
        <StepHeader
          ...
          description="Você não selecionou nenhuma tipografia como 'Gostei'. Volte ao passo anterior para selecionar pelo menos uma."
        />
      </div>
    );
  }
